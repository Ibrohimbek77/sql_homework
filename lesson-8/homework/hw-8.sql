...
SELECT 
	...
	CASE
		...
		WHEN Price BETWEEN 101 AND 500 THEN 'Mid'
		ELSE 'High'
	END AS PriceGroup
FROM Products
ORDER BY PriceGroup 
...
FROM (SELECT year, population FROM city_population) AS tempTable
PIVOT(
	SUM(population) FOR year IN([2013],[2012])
)AS years
...
FROM (SELECT district_name, population FROM city_population) AS temp
PIVOT(
...
SELECT 
	...
	TotalAmount
FROM Invoices
ORDER BY TotalAmount DESC OFFSET 0 ROWS FETCH NEXT 3 Rows only 
