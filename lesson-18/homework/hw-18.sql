...
--since current month not in the table I''l use MONTH(MAX(SaleDATE)) AND YEAR(MAX(SalesDate))
...
WHERE MONTH(SaleDate) = (SELECT MONTH(MAX(SaleDATE)) FROM Sales) AND YEAR(SaleDATE) = (SELECT YEAR(MAX(SaleDATE)) FROM Sales)
...
SELECT dbo.fn_IsPrime(71) AS isPrime
...
DECLARE @Start INT =  1
DECLARE @End INT = 10
SELECT * FROM fn_GetNumbersBetween(@Start, @End)
...
INSERT INTO Employee VALUES(1, 100),(2,200),(3,300),(4,400),(5,360)

SELECT dbo.getNthHighestSalary(20)
...
SELECT * FROM GuyWithMostFriends();
